      ...
      
      {
		...
		return view('placeholder ',['title'=> 'no blogs', 'info'=>'there is no blog with that Id ']);	 
		...
		else
		      {
					  ...
					  $this->imageExists = file_exists(  public_path('blogImages/'.$result->image)            );                             
	               ...
	               elseif($this->imageExists ==False)
	               
	               {
					$deleted = DB::table('blogs')
					  ->where('id', $this->Id)
					  ->delete();
					  return view('placeholder ',['title'=>'delete blogs ', 'info'=>'looks like there was a missing image for blog but  result of db is'.$deleted  ]);    
...
{
	  return redirect()->action([\App\Http\Controllers\Admin::class, 'blackcat']);
}
